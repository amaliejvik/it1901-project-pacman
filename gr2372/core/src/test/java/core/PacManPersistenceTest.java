...
public class PacManPersistenceTest {
  ...
  public void persistanceTest() {
    ...
    List<PacManUser> testScores = PacmanPersistence.fetchHighscore("src/test/java/core/JSON/testScores.json");
    PacManUser userFromScore = testScores.get(0);

    assertTrue(userFromScore.getUsername().equals("testUser"), "Username written to file is not the same as username read from file");
    assertTrue(userFromScore.getScore() == 100, "Score written to file is not the same as score read from file");
